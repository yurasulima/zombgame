...
public class Map implements IBaseUi {
    ...
    public void updateTiles() {
        try {
            BufferedImage grass = ImageIO.read(getClass().getResourceAsStream("/textures/blocks/grass00.png"));
            for (int x = 0; x < worldInfo.getSize(); x++) {
                for (int y = 0; y < worldInfo.getSize(); y++) {
                    Tile holders = map[0][x][y];
                    System.out.println("holders = " + holders.name);
                    if (holders.name.equals("earth")) {
                        System.out.println("holders = ");
                        Tile newTile = new Tile(grass, "grass");
                        newTile.setPos(x, y);
                        map[0][x][y] = newTile;
                    }
                }
